<template>
  <div id="Report">
    <!-- <hr /> -->
    <!-- <br /> -->
    <!-- <button v-on:click="report()">点我。。。。报道。。。</button>
    <br />
    <hr />
    <br />
    时间为: {{report_time}}
    ...
    <br />
    {{getSeatTotal()}}
    <button @click="getSeatTotal()">获取座位总数</button>-->
    <ul>
      <li :key="key" v-for="(item,key) in this.seats">{{item}}</li>
    </ul>
    <div class="certify">
      <button @click="choseSeatLater()">确认签到</button>
    </div>
    <div class="showArea">
      <!-- 用户操作之后的展示区-->
      <p>您选择的座位为: {{choseSeat}}</p>
      <!-- 报道结果展示区 -->
      <div class="report_result">
        <div class="result">
          <p>您的报道结果是:</p>
          <div class="seat_reuslt">
            <p>{{report_result}}</p>
          </div>
        </div>
      </div>
  ...
  methods: {
    ...
    async updateSeat() {
      ...
      this.getSeatTotal()
        ...
        .then(res => {
          let lis = document.querySelectorAll("li"); // 获取页面的座位
          // 如果座位有人坐了，会标记颜色
          let params = new URLSearchParams();
...
@import url("../assets/less/report.less");
</style>
...
let goInClass = {
  ...
  methods: {
    ...
    fnOpenDoorAnimation() {
      this.percentDoor = this.percentDoor + 3;
      if (this.percentDoor == 180) {
        this.eleDoorBox.classList.add("active");
        return;
      }
      // 门打开
      this.eleDoor.style.transform = `rotateY(${(-90 * this.percentDoor) /
        100}deg)`;
      this.eleLight.removeAttribute("hidden");
      // 光线变化
      this.eleLight.style.opacity = 0.8 - (0.9 * this.percentDoor) / 100;
      setTimeout(this.fnOpenDoorAnimation, 1);
    }
  },
  computed: {
    week: {
  ...
  mounted() {
    ...
    // 门，光线
    this.eleDoor = document.querySelector("#door");
    this.eleLight = document.querySelector("#light");
    this.eleDoorBox = document.querySelector("#doorBox");
  }
};
export default goInClass;
